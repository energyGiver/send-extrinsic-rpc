...
async fn xcm_transfer_1() {
    ...
    for url in urls {
        ...
        tx_manager
            .send_xcm(encoded_data.clone(), AccountKeyring::Alice)
            .await;
    }
}

// Bob in parachain-template-node -> Alice in infra-asset-system
async fn xcm_transfer_2() {
    ...
    let urls = vec!["http://localhost:10501"];

    for url in urls {
        let tx_manager = TxManager::new(url.to_string());
        tx_manager
            .send_xcm(encoded_data.clone(), AccountKeyring::Bob)
            .await;
    }
}
